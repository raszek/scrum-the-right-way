...
readonly class AddIssueDependencyEventRenderer implements IssueEventRendererInterface
{
    ...
    {
        return AddIssueDependencyEvent::class;
    }
    ...
    private function renderIssue(Event $event, Issue $issue, Issue $dependency): string
    {
        ...
        return sprintf(
            '<b>%s</b> has added dependency <a href="%s">%s</a>',
            $user->getFullName(),
            $dependencyUrl,
    ...
    private function render(Event $event, Issue $issue, Issue $dependency): string
    {
        ...
        return sprintf(
            '<b>%s</b> has added to <a href="%s">issue</a> dependency <a href="%s">%s</a>',
            $user->getFullName(),
            $issueUrl,
            $dependencyUrl,
            $dependency->getShortTitle()
