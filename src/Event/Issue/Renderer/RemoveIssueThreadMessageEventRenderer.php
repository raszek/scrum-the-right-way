...
readonly class RemoveIssueThreadMessageEventRenderer implements IssueEventRendererInterface
{
    ...
    private function renderIssue(Event $event, Issue $issue, ThreadMessage $threadMessage): string
    {
        ...
        return sprintf(
            '<b>%s</b> has removed thread message <a href="%s">%s</a>',
            $user->getFullName(),
            $threadUrl,
    ...
    private function render(Event $event, Issue $issue, ThreadMessage $threadMessage): string
    {
        ...
        return sprintf(
            '<b>%s</b> has removed from <a href="%s">issue</a> thread message <a href="%s">%s</a>',
            $user->getFullName(),
            $issueUrl,
            $threadUrl,
            $threadMessage->getIssueTitle()
